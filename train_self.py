...
def get_args():
    ...
    parser.add_argument("--q_network_iter", type=int, default=200, help='每隔一定时间，mainNet的参数拷贝到targetNet')
...
def train(opt):
    ...
    mainNet = DeepQNetwork()
    targetNet = DeepQNetwork()
    if os.path.isdir(opt.log_path):
        shutil.rmtree(opt.log_path)
    os.makedirs(opt.log_path)
    writer = SummaryWriter(opt.log_path)
    optimizer = torch.optim.Adam(mainNet.parameters(), lr=opt.lr)
    ...
    while iter < opt.num_iters:
        ...
        mainNet.train()
        current_prediction_batch = mainNet(state_batch)
        targetNet.eval()
        with torch.no_grad():
            next_prediction_batch = targetNet(next_state_batch)
        ...
        if (iter+1) % 1000000 == 0:
            torch.save(mainNet, "{}/flappy_bird_{}".format(opt.saved_path, iter+1))
        if (iter+1)%opt.q_network_iter == 0:
            targetNet.load_state_dict(mainNet.state_dict())
    torch.save(mainNet, "{}/flappy_bird".format(opt.saved_path))
