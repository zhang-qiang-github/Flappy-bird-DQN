...
def get_args():
    ...
    parser.add_argument("--ShowUI", type=bool, default=False, help='是否显示UI，不显示UI能加速')
...
def train(opt):
    ...
    model = DeepQNetwork()
    # model = torch.load('./trained_models/flappy_bird_100000')
    ...
    game_state = FlappyBird(opt.ShowUI)
    ...
    while iter < opt.num_iters:
        ...
        if (iter+1) % 10000 == 0:
            ...
            game_state_t = FlappyBird()

            image, reward, terminal = game_state_t.next_frame(0)
            image = pre_processing(image[:game_state.screen_width, :int(game_state.base_y)], opt.image_size,
                                   opt.image_size)
            image = torch.from_numpy(image)
            if torch.cuda.is_available():
                model.cuda()
            ...
            writer.add_scalar('Test/Reward', r, iter)
